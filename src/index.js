...
if (Constant.BotToken && Constant.BotToken != "") {
  try {
    ...
    console.log(
      "Invalid bot token found. Please update your configuration.yaml"
    );
  }
} else {
...
function onReady() {
  ...
  Commands.ReadEmails();
  bot.channels.fetch(Constant.ChannelSpam).then((channel) =>
    channel.send(`Clocking in!`).then((msg) => {
      msg.delete({ timeout: Constant.TempMessageTimeout });
    })
  );
...
async function onMessage(message) {
  if (
    message.author.bot ||
    !message.guild ||
    !Constant.Prefixes.some((pref) => message.content[0] == pref)
  )
    return;
  ...
  if (found_command) {
    message.channel.send(
      Embeds.SimpleEmbed(prefix + cmd, message, found_command.value)
    );
    return;
  }
  ...
  switch (cmd) {
    ...
    case "spoiler":
      message.channel.send(
        Embeds.SimpleEmbed(prefix + cmd, message, Commands.Spoiler())
      );
      break;
    case "lookup":
      var book_result = await Book.BookInfo(args);
      if (book_result) {
        var book_embed = await Embeds.BookEmbed(
          prefix + cmd + " " + args.join(" "),
          message,
          book_result,
          null,
          args.join(" ")
        ...
        message.channel
          .send(`There was an error retrieving results`)
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
          });
      }
    ...
    case "uptime":
      message.channel.send(
        Embeds.SimpleEmbed(prefix + cmd, message, Commands.Uptime(), "Uptime")
      );
      break;
    case "🏓":
    case "ping":
      message.channel.send(
        Embeds.SimpleEmbed(prefix + cmd, message, "Pong! 🏓")
      );
      break;
    case "kindle":
      message.channel.send(
        Embeds.KindleSendEmbed(prefix + cmd, message, Constant.LibrarianEmail)
      );
      break;
    case "addemail":
      try {
        Commands.AddEmail(message.author.id, args[0]);
        Commands.SendEmail(args[0], "current");
        message.channel.send(`Email added`).then((msg) => {
          msg.delete({ timeout: Constant.TempMessageTimeout });
        });
      } catch (e) {
    ...
    case "delemail":
      try {
        ...
        message.channel.send(`Email deleted`).then((msg) => {
          msg.delete({ timeout: Constant.TempMessageTimeout });
        });
      } catch (e) {
    ...
    case "checkemail":
      try {
        ...
        if (email) {
          message.author.send(
            `The current email we have on file is \`${email.email}\``
          );
        } else {
    ...
    case "suggest":
        ...
        if (suggestion_message.embeds && suggestion_message.embeds[0]) {
          suggestion_message.embeds[0].fields.forEach((field) => {
            if (field.name == "Suggestor") {
              if (field.value.includes(message.author.id) && !is_admin) {
                valid = false;
              }
            }
          });
      ...
      if (valid) {
        ...
        if (book_result) {
          var user_fields = [
            { name: "Suggestor", value: "<@" + message.author.id + ">" },
          ];
          var book_embed = await Embeds.BookEmbed(
            prefix + cmd + " " + args.join(" "),
            message,
        ...
        message.channel
          .send(
            `I'm sorry, you already have a suggestion listed for this round`
          )
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
          });
    ...
    case "addcom":
      ...
      try {
        Commands.AddCommand({
          name: potential_command_name,
          value: args.join(" "),
        });
        message.channel
          .send(`Command ${potential_command_name} added`)
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
          });
      } catch (e) {
    ...
    case "delcom":
      ...
      message.channel
        .send(`Command ${potential_command_name} deleted`)
        .then((msg) => {
          msg.delete({ timeout: Constant.TempMessageTimeout });
        });
      break;
    case "editcom":
      ...
      message.channel
        .send(`Command ${potential_command_name} updated`)
        .then((msg) => {
          msg.delete({ timeout: Constant.TempMessageTimeout });
        });
      break;
    case "selectbook":
      if (is_admin) cycleBookSelection(args ? args.join(" ") : null);
      break;
    case "setdownload":
      var valid =
        is_admin && args[0].includes("http") && args[0].includes("mobi");
      ...
      message.channel
        .send(
          Embeds.SimpleEmbed(
            prefix + cmd,
            message,
            valid ? "Current download updated" : "Something went wrong"
          )
        )
        .then((msg) => {
          msg.delete({ timeout: Constant.TempMessageTimeout });
    ...
    case "sendemail":
      ...
      if (recipient.includes("@") && attachment.includes("http")) {
        try {
          ...
          message.channel
            .send(Embeds.SimpleEmbed(prefix + cmd, message, "Email sent"))
            .then((msg) => {
              msg.delete({ timeout: Constant.TempMessageTimeout });
            });
        } catch {
        ...
        message.channel
          .send(
            Embeds.SimpleEmbed(
              prefix + cmd,
              message,
              "Email not sent. Could not verify arguments."
            )
          )
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
    ...
    case "sendemailall":
      ...
      if (attachment.includes("http")) {
        try {
          ...
          message.channel
            .send(Embeds.SimpleEmbed(prefix + cmd, message, "Emails sent"))
            .then((msg) => {
              msg.delete({ timeout: Constant.TempMessageTimeout });
            });
        } catch {
        ...
        message.channel
          .send(
            Embeds.SimpleEmbed(
              prefix + cmd,
              message,
              "Emails not sent. Could not verify arguments."
            )
          )
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
        ...
        message.channel
          .send(
            Embeds.SimpleEmbed(
              prefix + cmd,
              message,
              "Emails not sent. Could not verify attachment."
            )
          )
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
    ...
    case "react":
      ...
      try {
        message.channel.messages
          .fetch(msg_id)
          .then((msg) => msg.react(args[0]));
      } catch {
        message.channel
          .send(
            Embeds.SimpleEmbed(prefix + cmd, message, "Something went wrong...")
          )
          .then((msg) => {
            msg.delete({ timeout: Constant.TempMessageTimeout });
          });
    ...
    case "restart":
      if (is_admin) {
        message.channel
          .send(Embeds.SimpleEmbed(prefix + cmd, message, "Restarting..."))
          .then((msg) => {
      ...
      break;
    default:
      message.channel.send(`Command not found`).then((msg) => {
        msg.delete({ timeout: Constant.TempMessageTimeout });
      });
      break;
  }
...
async function onReactionAdd(reaction, user) {
  ...
  switch (reaction.emoji.name) {
          ...
          reaction.message.embeds[0].fields.forEach((field) => {
            if (field.name == "Suggestor") {
              if (field.value.includes(user.id)) {
                var bot_reaction = reaction.message.reactions.cache.filter(
                  (reaction_filter) => reaction_filter.me
                );
                del =
                  reaction.message.author.id == Constant.UserBot && is_admin;
                if (!bot_reaction.get("✅")) {
                  del = true;
                } else {
                  del = false;
    ...
    case "▶️":
      if (
        reaction.message.channel.id == Constant.ChannelUpcomingBook &&
        is_admin
      ) {
        cycleBookSelection();
      }
...
function userHasRole(user_id, role_name) {
  return bot.guilds.cache
    .get(Constant.ServerId)
    .members.cache.get(user_id)
    .roles.cache.some((role) => role.name === role_name);
}
...
async function selectBook(month_override = null) {
  ...
  messages = messages.filter(
    (message_f) =>
      !message_f.reactions.cache
        .filter((reaction_filter) => reaction_filter.me)
        .get("✅")
  );
    ...
    channel.send(
      messages
        ...
        .embeds[0].addField(
          ...
          month_override
            ? month_override
            : Constant.MonthNames[today.getMonth()]
        )
    );
  }
}
...
async function clearSuggestions() {
  var channel = await bot.channels.fetch(Constant.ChannelBookSuggestions);
  var messages = await channel.messages.fetch({ limit: 10 });
  messages = messages.filter((message_f) =>
    message_f.reactions.cache
      .filter((reaction_filter) => reaction_filter.me)
      .get("✅")
  );
...
async function cycleBookSelection(month_override = null) {
  bot.channels.fetch(Constant.ChannelCurrentBook).then((channel) => {
    channel.messages.fetch({ limit: 1 }).then((message) => {
      ...
      if (current_book_message) {
        bot.channels
          .fetch(Constant.ChannelPastBooks)
          .then((channel) => channel.send(current_book_message.embeds[0]));
        current_book_message.delete();
      }
    });

    bot.channels.fetch(Constant.ChannelUpcomingBook).then((channel) => {
      channel.messages.fetch({ limit: 1 }).then((message) => {
        var upcoming_book_message = message.values().next().value;
        if (upcoming_book_message) {
          bot.channels
            .fetch(Constant.ChannelCurrentBook)
            .then((channel) => channel.send(upcoming_book_message.embeds[0]));
          var next_month =
            Constant.MonthNames.indexOf(
              upcoming_book_message.embeds[0].fields.filter(
                (field) => field.name === "Monthly Selection"
              )[0].value
            ) + 1;
          selectBook(Constant.MonthNames[next_month > 11 ? 0 : next_month]);
          upcoming_book_message.delete();
        } else {
          selectBook();
        }
      });
